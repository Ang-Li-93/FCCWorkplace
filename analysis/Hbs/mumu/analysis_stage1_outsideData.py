...
class RDFanalysis():
    ...
    def analysers(df):
        ...
        #df = df.Alias("Lepton0",            "Muon_objIdx")
        #df = df.Alias("MCRecoAssociations0", "_MCRecoAssociations_rec")
        #df = df.Alias("MCRecoAssociations1", "_MCRecoAssociations_sim")
        #df = df.Alias("Particle0",           "_Particle_parents")
        #df = df.Alias("Particle1",           "_Particle_daughters")
        df = df.Define("Lepton0",
            "ROOT::VecOps::RVec<int> r; for(auto& x : Muon_objIdx) r.push_back(x.index); return r;")
        df = df.Define("MCRecoAssociations0",
            "ROOT::VecOps::RVec<int> r; for(auto& x : _MCRecoAssociations_rec) r.push_back(x.index); return r;")
        df = df.Define("MCRecoAssociations1",
            "ROOT::VecOps::RVec<int> r; for(auto& x : _MCRecoAssociations_sim) r.push_back(x.index); return r;")
        df = df.Define("Particle0",
            "ROOT::VecOps::RVec<int> r; for(auto& x : _Particle_parents) r.push_back(x.index); return r;")
        df = df.Define("Particle1",
            "ROOT::VecOps::RVec<int> r; for(auto& x : _Particle_daughters) r.push_back(x.index); return r;")
