...
def calculate_multiclass_BDT_input_numbers(mode_names, df, eff, xsec, class_mapping, frac):
    ...
    # 1. Calculate the total physical "weight" (yield) for each class
    # This tells us the relative proportions of modes *within* their own class
    class_xsec_totals = {0: 0, 1: 0, 2: 0, 3:0, 4:0}
    for cur_mode in mode_names:
        c = class_mapping[cur_mode]
        # Yield is proportional to efficiency * cross-section
        #class_xsec_totals now contains the total yield from each of the three groups
        class_xsec_totals[c] += eff[cur_mode] * xsec[cur_mode]

    # 2. Find the bottleneck class based on available simulated events
    # We still need to make sure we don't request more events than we actually have
    class_avail_counts = {0: 0, 1: 0, 2: 0, 3:0, 4:0}
    for cur_mode in mode_names:
        c = class_mapping[cur_mode]
    ...
    print(f"--> Target size for EACH of the 3 classes: {max_events_per_class} events")

    # 3. Distribute the budget inside each class using cross-section ratios
    N_BDT_inputs = {}
    for c_target in [1,3]:
    ...
    for c_target in [0,2,4]:
        #We want the rare decays (group 2) to be divided equally regardless of cross section
        modes_in_class = [m for m in mode_names if class_mapping[m] == c_target]
